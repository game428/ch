...
function htmlTxt (item){
  ...
  '" readonly placeholder="请选择"/><i class="iconfont icon-tiaozhuan"></i></span></div><div class="delete_btn">删除此条</div></div>'
  return html;
}
...
$(".js_five").click(function(){
  if($(".five_education_list").children(".step_cont").length < 1){
    popupType = 1;
    showPopup("请添加教育经历")
    return;
  ...
  if(search.type != 2){
    for(var i = 0;i< education_num;i++){
      ...
      if(!sname || !/^[A-Za-z0-9\u4e00-\u9fa5]+$/.test(sname)){
        popupType = 1;
        updateState = 0;
        showPopup("请正确填写学校名称")
        return;
      }else if(!majors || !/^[A-Za-z0-9\u4e00-\u9fa5]+$/.test(majors)){
        popupType = 1;
        updateState = 0;
        showPopup("请填写所学专业")
        return;
    ...
    if(exp_num > 1 || cname1 || job1 || starttime1 || endtime1 || profile1){
      for(var i = 0;i< exp_num;i++){
        ...
        if(!starttime){
          ...
          return;
        }else if(!cname || !/^[A-Za-z0-9\u4e00-\u9fa5]+$/.test(cname)){
          popupType = 1;
          updateState = 0;
          showPopup("请填写公司名称")
          return;
        }else if(!job || !/^[A-Za-z0-9\u4e00-\u9fa5]+$/.test(job)){
          popupType = 1;
          updateState = 0;
          showPopup("请填写职位")
          return;
